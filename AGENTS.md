...
The local store, exported URL, and optional remote backup have distinct roles — do not conflate them.
...
- Gist backup is optional and serializes the complete local library to `md-checklist.json`.
- Gist writes are debounced by two seconds after local persistence.
- Validate remote data before replacing local entries during restoration.
- Keep the token local: never include it in the Gist payload, URL, DOM, logs, or errors.
- Network failures must never prevent local editing or persistence.
...
| Gist functions | Optional remote backup and restore |
...
- **Single file, zero dependencies, no build.** No CDN or npm. `fetch` is reserved for the optional GitHub Gist API.
